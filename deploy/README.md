...
Before starting the service, set `CHGRID_HOST_ORIGIN` in `server/.env` to the exact browser origin that will host Indiginous, for example:
...
```apache
ProxyPass        /indiginous/ws  ws://127.0.0.1:8765/indiginous/ws
ProxyPassReverse /indiginous/ws  ws://127.0.0.1:8765/indiginous/ws
ProxyPass        /indiginous/auth/session/  http://127.0.0.1:8765/indiginous/auth/session/
ProxyPassReverse /indiginous/auth/session/  http://127.0.0.1:8765/indiginous/auth/session/
```
