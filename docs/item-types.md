...
- A carried radio remote can tune connected radios/speakers in any location that contains a compatible radio or speaker, using comma/period, Ctrl+comma/period, or Ctrl+Shift+comma/period, and can raise/lower connected radio volume with Ctrl+Shift+Up/Down or Ctrl+Shift+U/D. The server only accepts these controls when the remote is actually carried by the user. Remote tuning preserves each radio's own `enabled` state, so an off radio stays off while still receiving the current station index for when it is turned back on. Remote volume changes are relative per speaker, so separate sub/low/mid/high balances stay separate instead of being flattened to one value.
...
- Alarm actions use louder generated spoken prompt assets with a short filtered intercom reflection for setup required, setup complete, access granted, access denied, and alarm triggered. These play spatially from the panel.
- `use` on a disarmed alarm reports that the house alarm is disarmed.
- `use` by an authorized signed-in account username speaks the allow prompt and does not trigger when `accessMethod=account`. With `accessMethod=account_keypad`, that authenticated identity must also submit the enrolled resident code. Display names are never credentials.
...
- Alarm attempts and impending/successful entries alert active occupants throughout the protected house scope and create targeted in-grid account notifications for enrolled/authorized owners. Owners who enabled their private server-backed ntfy identity topic receive the same events there. `waNotifyTarget` remains a hook until an approved WhatsApp action provider is wired.
- Exterior house doors may link to a panel through `accessAlarmItemId`. The server keeps unauthorized users outside. Valid residents enter immediately after verification. Valid guest-code entrants receive a ten-second warning, occupants are alerted, and then the server opens the door and moves the guest inside.
...
- Teleport pads gently drift one safe cardinal square at irregular intervals. They remain inside their assigned location and do not move onto blocking items.
...
- Normal pickup of a movable furniture surface always moves that surface plus everything resting on it, preserving surface identity and order while carried and after it is set down. Supported contents count as part of the furniture load rather than separate hand loads. Other items move alone unless attached/group pickup is requested. Wall-, ceiling-, window-, and fixture-mounted house objects are immovable.
