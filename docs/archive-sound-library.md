...
For Indiginous rooms and locations, check `Ambiance/` before generating new loops. It already has room and office candidates such as:
...
Indiginous widgets can use these as full HTTPS URLs in `emitSound` for ambience loops or `useSound` for one-shot item sounds. Example:
