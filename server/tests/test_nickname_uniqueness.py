...
def test_nickname_taken_is_case_insensitive() -> None:
    ...
    first_ws = _fake_ws()
    second_ws = _fake_ws()
    server.clients[first_ws] = ClientConnection(
        websocket=first_ws, id="1", nickname="Jage"
    )
    server.clients[second_ws] = ClientConnection(
        websocket=second_ws, id="2", nickname="Alice"
    )
