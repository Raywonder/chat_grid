...
def resolve_launch_url(arguments: list[str], default_url: str = DEFAULT_URL) -> str:
    ...
    links = [value for value in arguments if value.lower().startswith(("indiginous://", "indiginious://", "chatgrid://"))]
    if not links:
        return default_url
    outer = urlparse(links[-1])
    if outer.scheme.lower() not in {"indiginous", "indiginious", "chatgrid"} or outer.netloc.lower() != "connect":
        return default_url
    ...
    if not (
        ...
        or parsed.path == "/indiginious"
        or parsed.path.startswith("/indiginious/")
