...
def test_browser_auth_accepts_matching_loopback_callback() -> None:
    ...
    errors: list[str] = []
    flow = BrowserAuthFlow("https://blind.software", "https://blind.software/indiginous/", timeout_seconds=2)
    ...
    thread.join(2)
    assert results == [("https://blind.software/indiginous/", assertion)]
    assert errors == []
...
def test_browser_auth_rejects_wrong_state() -> None:
    ...
    thread.join(2)
    assert errors == ["The browser returned an invalid or expired Indiginous sign-in response."]


def test_browser_auth_requires_https_origin() -> None:
    try:
        BrowserAuthFlow("http://blind.software", "https://blind.software/indiginous/")
    except ValueError:
        return
