...
def test_browser_auth_accepts_matching_loopback_callback() -> None:
    ...
    errors: list[str] = []
    flow = BrowserAuthFlow("https://blind.software", "https://blind.software/endiginous/", timeout_seconds=2)
    ...
    thread.join(2)
    assert results == [("https://blind.software/endiginous/", assertion)]
    assert errors == []
...
def test_browser_auth_requires_https_origin() -> None:
    try:
        BrowserAuthFlow("http://blind.software", "https://blind.software/endiginous/")
    except ValueError:
        return
