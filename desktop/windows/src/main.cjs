  ...
  const template = [
    {
      ...
      submenu: [
        ...
        { label: 'Settings...', accelerator: 'Ctrl+,', click: () => mainWindow?.webContents.executeJavaScript('window.indiginousNativeOpenSettings?.()') },
        { label: 'Indiginous URL...', accelerator: 'Ctrl+Shift+U', click: promptForIndiginousUrl },
        { label: 'Cast to device...', accelerator: 'Ctrl+Shift+C', click: () => mainWindow?.webContents.executeJavaScript("window.dispatchEvent(new Event('indiginous-cast-to-device'));") },
        { type: 'separator' },
