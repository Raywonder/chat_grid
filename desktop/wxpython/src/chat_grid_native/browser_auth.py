"""System-browser authentication handoff for the Indiginous desktop client."""
...
class BrowserAuthFlow:
    """Receive one short-lived Blind Software assertion on loopback."""
    ...
    def __init__(self, server_origin: str, grid_url: str, timeout_seconds: float = 180.0) -> None:
        ...
        self.authorization_url = self.server_origin + "/?" + urlencode(
            {"route": "indiginous_client_auth_start", "callback": self.callback_url, "state": self.state}
        )
        ...
        class CallbackHandler(BaseHTTPRequestHandler):
            def do_GET(self) -> None:  # noqa: N802
                ...
                if valid:
                    ...
                    self._reply(200, "Indiginous sign-in complete", "Return to Indiginous. This browser tab can be closed.")
                else:
                    flow.error = "The browser returned an invalid or expired Indiginous sign-in response."
                    self._reply(400, "Indiginous sign-in failed", flow.error)
    ...
    def start(self, on_success: Callable[[str, str], None], on_error: Callable[[str], None]) -> threading.Thread:
        ...
        def worker() -> None:
            try:
                ...
                if self.assertion:
                    ...
                    on_error(self.error or "Browser sign-in timed out. Try again from Indiginous.")
            finally:
                self._server.server_close()

        thread = threading.Thread(target=worker, name="indiginous-browser-auth", daemon=True)
        thread.start()
        return thread
